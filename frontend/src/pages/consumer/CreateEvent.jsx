...
export default function CreateEvent() {
  ...
  useEffect(() => {
    api.get('/catalog').then(({ data }) => {
      ...
      setEventPresets(data.eventPresets);
  ...
  const detectLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ({ coords }) => {
        setEvent(e => ({ ...e, userLat: coords.latitude, userLng: coords.longitude }));
        setGeoLoading(false);
      },
      () => {
        setGeoLoading(false);
  ...
  const generate = async () => {
    ...
    setGenerating(true);
    try {
      const { data } = await api.post('/shopping-list/generate', {
      ...
      navigate('/consumer/shopping-list', { state: { result: data, event } });
    } catch (err) {
      alert(err.response?.data?.error || 'Error generando la lista');
    } finally {
      setGenerating(false);
    }
  };
  ...
  const canNext = () => {
    ...
    if (step === 1) return Object.values(event.selections).some(arr => arr.length > 0);
    ...
    <div className="page-container flex flex-col min-h-screen">
      ...
      <div className="bg-gradient-to-br from-brand-800 to-brand-700 px-6 pt-12 pb-6">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={logout} className="text-white/50 text-xs border border-white/20 rounded-lg px-3 py-1.5">
            Salir
          </button>
        </div>
        ...
        <div className="flex items-center gap-1">
          {STEPS.map((s, i) => (
            <div key={i} className="flex items-center gap-1 flex-1">
              <div className={`flex-1 h-1.5 rounded-full transition-all duration-300 ${i <= step ? 'bg-white' : 'bg-white/25'}`} />
              {i < STEPS.length - 1 && <div className={`w-4 h-1.5 rounded-full transition-all ${i < step ? 'bg-white' : 'bg-white/25'}`} />}
            </div>
          ))}
        </div>
        <div className="flex justify-between mt-1">
          {STEPS.map((s, i) => (
            <span key={i} className={`text-xs transition-colors ${i === step ? 'text-white font-semibold' : 'text-white/40'}`}>
              {s}
            </span>
          ))}
        </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {Object.entries(eventPresets).map(([key, preset]) => (
                ...
                >
                  ...
                  {event.type === key && (
                    <div className="mt-2 text-xs text-brand-600 font-medium">✓ Seleccionado</div>
                  )}
                </button>
              ))}
            </div>
        ...
        {step === 1 && (
          <div className="flex-1 flex flex-col">
            ...
            <p className="text-gray-500 text-sm mb-4">
              Pre-seleccionamos productos para tu {eventPresets[event.type]?.label}. Personalizalos.
            </p>
            ...
            <div className="flex gap-2 overflow-x-auto no-scrollbar mb-4 -mx-1 px-1">
              {allCategories.map(cat => {
                ...
                const isActive = activeCategory === cat || (!activeCategory && allCategories[0] === cat);
                return (
                  <button
                    key={cat}
            ...
            <div className="flex-1 overflow-y-auto -mx-1 px-1">
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(catalog)
                  .filter(([, p]) => p.category === (activeCategory || allCategories[0]))
                  .map(([name, product]) => {
                    const selected = (event.selections[product.category] || []).includes(name);
                    return (
                      <button
                        key={name}
                        onClick={() => toggleProduct(product.category, name)}
                        className={`p-3 rounded-2xl border-2 text-left transition-all active:scale-95 ${
                          selected ? 'border-brand-500 bg-brand-50' : 'border-gray-100 bg-white'
                        }`}
                      >
                        ...
                        <div className="text-xs text-gray-400 mt-0.5">
                          {product.perEvent
                            ? `${product.perEvent} ${product.unit}`
                            : `${product.perPerson} ${product.unit}/pers.`}
                        </div>
        ...
        {step === 2 && (
          <div className="flex-1 flex flex-col items-center justify-center text-center">
            ...
            <div className="flex items-center gap-6">
              <button
                onClick={() => setEvent(e => ({ ...e, guests: Math.max(1, e.guests - 1) }))}
                className="w-14 h-14 rounded-full bg-gray-100 text-gray-700 text-2xl font-bold active:scale-90 transition-all flex items-center justify-center"
              >
                −
              </button>
              ...
              <button
                onClick={() => setEvent(e => ({ ...e, guests: e.guests + 1 }))}
                className="w-14 h-14 rounded-full bg-brand-700 text-white text-2xl font-bold active:scale-90 transition-all flex items-center justify-center"
              >
                +
              </button>
            </div>
            ...
            <div className="mt-10 w-full bg-brand-50 rounded-2xl p-4">
              <p className="text-sm font-semibold text-brand-700 mb-2">Resumen del evento</p>
              <div className="flex justify-between text-sm">
                <span className="text-gray-600">Tipo</span>
                <span className="font-medium">
                  {eventPresets[event.type]?.emoji} {eventPresets[event.type]?.label}
                </span>
              </div>
              <div className="flex justify-between text-sm mt-1">
                <span className="text-gray-600">Productos seleccionados</span>
                <span className="font-medium">
                  {Object.values(event.selections).flat().length}
                </span>
              </div>
            </div>
          </div>
        ...
        {step === 3 && (
          <div className="flex-1">
            ...
            <p className="text-gray-500 text-sm mb-6">
              Buscamos ofertas en un radio alrededor tuyo
            </p>
              ...
              {event.userLat ? (
                <div className="text-center">
                  ...
                  <p className="text-green-600 text-xs mt-1">
                    {event.userLat.toFixed(4)}, {event.userLng.toFixed(4)}
                  </p>
                </div>
              ) : (
                <div className="text-center text-gray-400">
                  <div className="text-4xl mb-2">🗺️</div>
                  <p className="text-sm">Sin ubicación aún</p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <input
                ...
                step={0.5}
                value={event.radius}
                onChange={(e) => setEvent(ev => ({ ...ev, radius: parseFloat(e.target.value) }))}
            ...
            <div className="mt-6 bg-brand-50 rounded-2xl p-4 text-sm text-brand-800">
              <p className="font-semibold mb-1">¿Cómo funciona?</p>
              <p className="text-brand-700/70 text-xs leading-relaxed">
                Buscamos todos los comercios registrados dentro de {event.radius} km de tu posición
                que tengan ofertas para tus productos. Generamos dos listas optimizadas para que
      ...
      <div className="px-6 pb-8 pt-4 bg-white border-t border-gray-100">
        <div className="flex gap-3">
          {step > 0 && (
            <button onClick={() => setStep(s => s - 1)} className="btn-secondary flex-none px-6 w-auto">
              ← Atrás
            </button>
          )}
          {step < STEPS.length - 1 ? (
            <button
              onClick={() => { setStep(s => s + 1); setActiveCategory(null); }}
              disabled={!canNext()}
            ...
            <button
              onClick={generate}
              disabled={generating || !event.userLat}
              className="btn-primary flex-1"
            >
