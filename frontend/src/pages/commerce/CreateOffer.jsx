...
export default function CreateOffer() {
  ...
  useEffect(() => {
    api.get('/catalog').then(({ data }) => setCatalog(data.catalog));
  }, []);
  ...
  useEffect(() => {
    setForm(f => ({ ...f, expiresAt: defaultExpiry() }));
  }, []);

  const filteredProducts = Object.keys(catalog).filter(name =>
  ...
  const submit = async (e) => {
    ...
    setLoading(true);
    try {
      await api.post('/offers', { ...form, userId: user.id, price: parseFloat(form.price) });
      navigate('/commerce');
    } catch (err) {
      setError(err.response?.data?.error || 'Error publicando la oferta');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="page-container flex flex-col min-h-screen">
      ...
      <div className="bg-gradient-to-br from-gray-900 to-gray-800 px-6 pt-12 pb-6">
        <button onClick={() => navigate('/commerce')} className="text-white/60 text-sm mb-4 flex items-center gap-1">
          ← Volver
        </button>
      ...
      <form onSubmit={submit} className="flex-1 px-6 pt-6 pb-8 space-y-5">
        ...
        <div>
          ...
          {form.productName && !customProduct ? (
            <div className="flex items-center gap-2 bg-brand-50 border-2 border-brand-300 rounded-2xl px-4 py-3">
              ...
              <button
                type="button"
                onClick={() => setForm(f => ({ ...f, productName: '' }))}
            ...
            <>
              ...
              {search.length > 0 && (
                <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-lg max-h-48 overflow-y-auto">
                  {filteredProducts.length > 0 ? (
                    filteredProducts.slice(0, 8).map(name => (
                      <button
                        key={name}
                        type="button"
                      ...
                      >
                        <span className="mr-2">{catalog[name]?.emoji}</span>
                        {name}
              ...
              <button
                type="button"
                onClick={() => { setCustomProduct(true); setForm(f => ({ ...f, productName: search || '' })); }}
        ...
        <div>
          ...
          <p className="text-xs text-gray-400 mt-1">Precio por unidad del producto (kg, lt, unidad, etc.)</p>
        </div>
        ...
        <div>
          ...
          <textarea
            name="description"
            placeholder="Ej: Precio por kilo, oferta de la semana, especial Navidad..."
        ...
        <div>
          <label className="text-sm font-semibold text-gray-700 mb-1.5 block">
            Válida hasta *
            <span className="text-xs text-gray-400 font-normal ml-1">(por la inflación 😅)</span>
          </label>
          <input
            name="expiresAt"
            type="date"
